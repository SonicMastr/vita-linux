...
{
	return !!(vita_uart_read32(port, VITA_UART_STATUS) &
		  VITA_UART_STATUS_TXREADY);
}

static bool vita_uart_dev_ready(struct uart_port *port)
{
	return !!(vita_uart_read32(port, VITA_UART_STATUS) &
		  VITA_UART_STATUS_DEVREADY);
}

static u32 vita_uart_read_cnt(struct uart_port *port)
{
	return vita_uart_read32(port, VITA_UART_READ_CNT) &
	       VITA_UART_READ_CNT_MASK;
}
...
static void vita_uart_tx_chars(struct uart_port *port)
{
	...
	while (!uart_circ_empty(xmit)) {
		...
		if (port->x_char) {
			vita_uart_write32(port, port->x_char,
					  VITA_UART_WRITE_FIFO);
			port->x_char = 0;
			port->icount.tx++;
			continue;
		}
		...
		vita_uart_write32(port, xmit->buf[xmit->tail],
				  VITA_UART_WRITE_FIFO);
...
static void vita_uart_set_termios(struct uart_port *port,
...
static int vita_uart_verify_port(struct uart_port *port,
...
static void vita_uart_console_putchar(struct uart_port *port, unsigned char ch)
{
	while (!vita_uart_tx_ready(port))
		cpu_relax();
...
static void vita_uart_console_write(struct console *co, const char *s,
				    unsigned int cnt)
{
...
static void vita_early_putchar(struct uart_port *port, unsigned char ch)
{
	while (!vita_uart_tx_ready(port))
		cpu_relax();
...
{
	struct vita_uart_port *port = container_of(
		to_delayed_work(work), struct vita_uart_port, rx_poll_work);
