...
static int syscon_ctrl_device_reset(struct vita_syscon *syscon,
				    unsigned int param_1, unsigned int param_2)
{
	return syscon->short_command_write(syscon, 0x88F,
					   param_2 | (param_1 << 8), 3);
}

static int
syscon_get_touchpanel_device_info(struct vita_syscon *syscon,
...
static int syscon_get_touchpanel_device_info_ext(
	struct vita_syscon *syscon,
...
static int syscon_get_touchpanel_unk_info_front(struct vita_syscon *syscon,
						u16 *data)
{
	int ret;
...
static int syscon_get_touchpanel_unk_info_back(struct vita_syscon *syscon,
					       u16 *data)
{
	int ret;
	u8 rx[SYSCON_RX_HEADER_SIZE + sizeof(u16) + 1];
...
static int syscon_touch_set_sampling_cycle(struct vita_syscon *syscon,
					   int cycles_front, int cycles_back)
{
...
static int touch_init(struct vita_syscon_ts *ts)
{
	...
	if (baryon_version < 0x1000600) {
		ret = touch_read_panel_info_1(syscon, &ts->vendor.front,
					      &ts->vendor.back);
	} else {
		ret = touch_read_panel_info_2(syscon, &ts->vendor.front,
					      &ts->vendor.back);
	}
	if (ret < 0)
		return ret;
...
static inline bool touch_parse_finger(struct touch_finger *finger,
				      const u8 *data)
{
	u8 id = data[0];
	if (id & 0x80)
...
static int touch_read(struct vita_syscon_ts *ts, int port_mask,
		      struct touch_report *report)
{
	...
	ret = syscon->command_read(syscon, 0x300 + cmd_lo, buffer,
				   sizeof(buffer));
	if (ret < 0)
		return ret;
