...
static const struct mfd_cell vita_syscon_devs[] = {
	{ .name = "vita-syscon-ts", .of_compatible = "vita,syscon-ts" },
...
static int vita_syscon_transfer(struct vita_syscon *syscon, u8 *tx, void *rx,
				int rx_size)
{
	...
	do {
		...
		if (!timeout) {
			dev_warn(&spi->dev, "%s: RX IRQ timeout",
				 dev_name(&spi->dev));
			ret = -ETIMEDOUT;
			goto out;
		}
...
static int vita_syscon_short_command_write(struct vita_syscon *syscon, u16 cmd,
					   u32 data, int cmd_len)
{
...
static int vita_syscon_probe(struct spi_device *spi)
{
	...
	u8 baryon_version[SYSCON_RX_HEADER_SIZE +
	...
	syscon =
		devm_kzalloc(&spi->dev, sizeof(struct vita_syscon), GFP_KERNEL);
	if (!syscon)
		return -ENOMEM;
	...
	ret = devm_request_irq(&spi->dev, irq, vita_syscon_rx_gpio_irq_handler,
			       0, "vita-syscon-gpio-rx", syscon);
	if (ret) {
	...
	ret = vita_syscon_command_read(syscon, 1, baryon_version,
				       sizeof(baryon_version));
	if (ret < 0) {
		return ret;
	}
	...
	memcpy(&syscon->hardware_info, &hw_info[SYSCON_RX_DATA],
	       sizeof(syscon->hardware_info));
	...
	memcpy(syscon->hardware_flags, &hw_flags[SYSCON_RX_DATA],
	       sizeof(syscon->hardware_flags));

	return devm_mfd_add_devices(syscon->dev, PLATFORM_DEVID_NONE,
				    vita_syscon_devs,
				    ARRAY_SIZE(vita_syscon_devs), NULL, 0,
				    NULL);
}
