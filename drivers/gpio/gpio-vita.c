...
static struct irq_chip vita_gpio_irqchip = { .name = "vita-gpio",
					     .irq_mask = vita_gpio_irq_mask,
					     .irq_unmask = vita_gpio_irq_unmask,
					     .irq_set_type =
						     vita_gpio_irq_set_type };
...
static int vita_gpio_probe(struct platform_device *pdev)
{
	...
	ret = bgpio_init(&vgpio->gc, dev, 4, vgpio->regs + VITA_GPIO_READ,
			 vgpio->regs + VITA_GPIO_SET,
			 vgpio->regs + VITA_GPIO_CLEAR,
			 vgpio->regs + VITA_GPIO_DIRECTION, NULL,
	...
	if (IS_ERR(vgpio->clk)) {
		...
		dev_err(dev, "Failed to retrieve peripheral clock, %d\n", ret);
		return ret;
	}

	ret = clk_prepare_enable(vgpio->clk);
	if (ret) {
		dev_err(dev, "Failed to enable the peripheral clock, %d\n",
			ret);
		return ret;
	}
	...
	girq->num_parents = 1;
	girq->parents =
		devm_kcalloc(dev, 1, sizeof(*girq->parents), GFP_KERNEL);
	if (!girq->parents) {
		ret = -ENOMEM;
		goto err_reset;
	}
